...
export const articleRouter = createCrudRouterWithCustom(
  ...
  (t) => ({
    publish: t.procedure.input(PublishSchema).mutation(async ({ ctx, input }) => {
      // 自定义发布逻辑
    }),
  }),
);
...
// 状态转换验证
this.stateMachine.validateTransition(entity.status, 'PUBLISHED', '资源');
...
export class ScheduledTaskService {
  ...
  async handleScheduledTask() {
    ...
    try {
      ...
      // 3. 推送任务到队列（异步处理）
      await this.queue.addBulk(records.map((r) => ({ name: 'process', data: { id: r.id } })));
    } finally {
      await this.redis.releaseLock('scheduler:task-name', lock);
    }
  }
